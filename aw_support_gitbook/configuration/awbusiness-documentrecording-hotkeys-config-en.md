...
The table below documents the hotkeys that can be used to access particular functions from the ribbon more quickly in the document recording. Since these hotkeys change depending on the active tab, they are grouped per active dialog (header data, items, shapes, etc.). Under the General entry are hotkeys that are not assigned buttons on the ribbon and thus can only be reached using the keyboard.
